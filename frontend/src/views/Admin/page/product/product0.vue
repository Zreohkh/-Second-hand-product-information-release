          ...
          <el-table-column label="不通过">
            ...
            <template slot-scope="scope">
              <el-button
                ...
                size="mini"
                icon="el-icon-edit"
                @click="reject(scope.row.itemInfo.pid)"
              >不通过</el-button>
            </template>
...
export default {
  ...
  methods: {
    ...
    getProductList() {
      const copy = Object.assign(this.queryInfo, { type: 0 });
      console.log(copy)
      productList(copy)
        .then((res) => {
  ...
  approve(id) {
    ...
    async reject(id) {
      review({id:id,rcon:2,tableName:'product'}).then(res=>{
          console.log(res.data.data);
          this.getProductList();
      })
